...
namespace Yoga.VisionMix.Scheduling
{
    public class RunTheadData
    {
        ...
        bool result_Acceptable = true;
        public bool Result_Acceptable
        {
            get
            {
                lock (lockObj)
        ...
        public void RunAllTool()
        {
            ...
            try
            {
                        ...
                        && datSend != string.Empty)
                    {
                        ...
                        lock (lockobj1)
                        {
                            if (Result_Acceptable)
                            {
                                int id = 0;
                                //获取项目名称
                                string prj_name = Path.GetFileNameWithoutExtension(UserSetting.Instance.ProjectPath);
                                #region 产品ID号                                
                                if (Regex.Match(prj_name, "HSA-F M16.102").Success)
                                {
                                    id = 1;
                                }
                                else if (Regex.Match(prj_name, "HSA-F M16.137").Success)
                                {
                                    id = 2;
                                }
                                else if (Regex.Match(prj_name, "HSA-F M16.182").Success)
                                {
                                    id = 3;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.102").Success)
                                {
                                    id = 4;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.117").Success)
                                {
                                    id = 5;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.137").Success)
                                {
                                    id = 6;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.182").Success)
                                {
                                    id = 7;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.100").Success)
                                {
                                    id = 8;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.120").Success)
                                {
                                    id = 9;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.140").Success)
                                {
                                    id = 10;
                                }
                                else if (Regex.Match(prj_name, "HSA M16.185").Success)
                                {
                                    id = 11;
                                }
                                else if (Regex.Match(prj_name, "HSA M12.100").Success)
                                {
                                    id = 12;
                                }
                                else if (Regex.Match(prj_name, "HSA M12.115").Success)
                                {
                                    id = 13;
                                }
                                else if (Regex.Match(prj_name, "HSA M12.100-BW").Success)
                                {
                                    id = 14;
                                }
                                else
                                {
                                    id = 0;
                                    Util.Notify(Level.Err, string.Format("没有这个产品型号"));
                                }
                                #endregion
                                string temp = "ID:" + id;
                                Match match = Regex.Match(datSend, "ID:[1-9]");
                                datSend=Regex.Replace(datSend, "ID:[1-9]", temp);
                                Util.Notify(string.Format("发送结果: {0}", datSend));
                                autoUnit.RunCommWriteDataThread.WriteData(datSend);                                
                            }
                            else
                            {
                                Util.Notify(Level.Err, string.Format("超时跳过发送结果"));
                            }
                        }
                    }
                    else
                ...
                autoUnit.Invoke(new Action<HWndCtrl, List<ToolBase>, bool>((h, l, f) =>
              {
                ...
                // ShowResult(hWndCtrl, runToolList, runningResultFlag);
                Thread.Sleep(30);
                #endregion
            ...
            {
                Util.Notify(Level.Err, string.Format("图像处理异常"));
            }
            finally
            {
        ...
        private void Camera_ImageEvent(object sender, ImageEventArgs e)
        {
            ...
            {
                //硬件触发
                TrigerRun(e, 250, e.CameraIndex, true);
            }
